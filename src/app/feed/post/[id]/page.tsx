...
export default function PostDetailPage({ params }: { params: { id: string } }) {
  ...
  const [post, setPost] = useState<FeedPost>({
    ...postData,
    comments_count: postData.replies_count,
    likes_count: Math.floor(Math.random() * 100),
    image_url: mockImages[Math.floor(Math.random() * mockImages.length)],
  });
  const [comments, setComments] = useState<FeedComment[]>(
    commentsData.map((r) => ({ ...r, likes_count: Math.floor(Math.random() * 20) }))
  );
        ...
        >
          <div className="p-4">
            <div className="flex items-center gap-3 mb-3">
              <img src={getAvatarUrl(post.username)} alt={post.username} className="h-10 w-10 rounded-full" />
              <div>
